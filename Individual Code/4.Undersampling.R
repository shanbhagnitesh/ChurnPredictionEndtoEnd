...
create_output_file_path <- function(cv_id,cvrun_id,under_rate){
  fileTrainPathPred <-
    paste0("C:/Users/nshanbhag/Desktop/BigDataAnalytics/Machine learning for Marketing/Individual Code/output_predicted/",
           "unb2_cv",cv_id,
           "train",cvrun_id,
  ...
  filePathAUC <-
    paste0("C:/Users/nshanbhag/Desktop/BigDataAnalytics/Machine learning for Marketing/Individual Code/output_auc/",
           "unb2_cv",cv_id,
           "run",cvrun_id,
...
calculate_auc <- function(under_obs, test_p){
  ...
  pred <- prediction(test_p[,under_obs],test_p[,"churn"])
...
for(cv_id in c(1:5)){
  for(cvrun_id in c(1:2)){
    for(under_rate in seq(0.4,0.5, by = 0.1)){
      ...
      # Subset the data on churn 0 or 1
      dsn_train_0 <- subset(dsn_train, churn == 0)
      dsn_train_1 <- subset(dsn_train, churn == 1)
      ...
      mylogit <- glm(churn ~ ., data = dsn_train, family = binomial)
      
      dsn_test_p <- data.frame(churn= dsn_test$churn)
      dsn_train_p <- data.frame(churn= dsn_train$churn)
      ...
      for(i in 1:20){
        mylogit <- glm(churn ~ ., data = dsn_train_f[[i]], family = binomial)
