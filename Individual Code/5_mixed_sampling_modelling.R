...
create_output_file_path <- function(cv_id,cvrun_id,under_rate){
  fileTrainPathPred <-
    paste0("C:/Users/nshanbhag/Desktop/BigDataAnalytics/Machine learning for Marketing/Individual Code/output_predicted - Mixed/",
           "unb2_cv",cv_id,
           "train",cvrun_id,
           "_p.csv")
  ...
  filePathAUC <-
    paste0("C:/Users/nshanbhag/Desktop/BigDataAnalytics/Machine learning for Marketing/Individual Code/output_auc - Mixed/",
           "unb2_cv",cv_id,
           "run",cvrun_id,
           "_AUC.csv")
...
for(cv_id in c(1:5)){
  for(cvrun_id in c(1:2)){
    ...
    data_balanced_both <- ovun.sample(churn ~ ., data = train, method = "both", p=0.5,N=nrow(train))$data
...
# Change only the Wanted columns, Paths and churn variable.