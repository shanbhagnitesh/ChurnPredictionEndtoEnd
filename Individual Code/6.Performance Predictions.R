...
calculate_performance_params <- function(under_obs, test_p){
  ...
  auc_val <- auc(test_p[,"churn"],test_p[,under_obs])
...
pred_files <- list.files("C:/Users/nshanbhag/Desktop/BigDataAnalytics/Machine learning for Marketing/Individual Code/output_predicted/")
pred_files <- paste0("C:/Users/nshanbhag/Desktop/BigDataAnalytics/Machine learning for Marketing/Individual Code/output_predicted/", pred_files)

pred_files_mixed <- list.files("C:/Users/nshanbhag/Desktop/BigDataAnalytics/Machine learning for Marketing/Individual Code/output_predicted - Mixed/")
pred_files_mixed <- paste0("C:/Users/nshanbhag/Desktop/BigDataAnalytics/Machine learning for Marketing/Individual Code/output_predicted - Mixed/", pred_files_mixed)
...
input_data <- function(path){
  ...
  df <- df[colnames(df) %in% c("churn","p_1")]
  df$index <- 1:nrow(df)
...
write.csv(final_predictions_test, file="C:/Users/nshanbhag/Desktop/BigDataAnalytics/Machine learning for Marketing/Individual Code/final_predictions_test.csv", row.names=F)
write.csv(final_predictions_train, file="C:/Users/nshanbhag/Desktop/BigDataAnalytics/Machine learning for Marketing/Individual Code/final_predictions_train.csv", row.names=F)
...
write.csv(final_performance, file="C:/Users/nshanbhag/Desktop/BigDataAnalytics/Machine learning for Marketing/Individual Code/final_performance1.csv", row.names=F)

# Create Gains and Lift Curve
pred_0.4 <- prediction(final_predictions_test$Undersampling_0.4, final_predictions_test$churn)
pred_0.5 <- prediction(final_predictions_test$Undersampling_0.5, final_predictions_test$churn)
pred_Mixed <- prediction(final_predictions_test$Mixed_Sampling, final_predictions_test$churn)
